let CfoDealCheckNew = async (browser, page, DealData) => {
    ...
    //await page.setViewport({width2, height2});
        ...
        //Ждём активную кнопку CFO
        xPath = `//a[@href="/cfo"][@class="router-link-exact-active router-link-active"]`;
        resOk = await WaitForElementIsPresentByXPath(3000, page, xPath);
        if (!resOk){
            tStr = `FAIL => Не вижу Активной кнопки (CFO)`;
            g_StrOutLog+= `${tStr}\n`;
        ...
        if (!resOk){
            tStr = `FAIL => проверка таблицы сделок (CFO)`;
            g_StrOutLog+= `${tStr}\n`;
            await console.log(tStr);
        ...
        await console.log(`CfoDealCheckNew OK`);
        //await TempStop(page);
...
module.exports.CfoDealCheckNew = CfoDealCheckNew;
